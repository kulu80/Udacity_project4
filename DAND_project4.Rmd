...
qplot(x =fixed.acidity, data = wht_wine_quality) + scale_x_continuous()
# Limit the x-axis to fit the data
qplot(x = fixed.acidity, data = wht_wine_quality, binwidth = 0.1) + scale_x_continuous(limits = c(3, 12), breaks = seq(3,12,1))
# Use ggplot to plot the same data and see the difference
ggplot(aes(x = fixed.acidity), data = wht_wine_quality) + geom_histogram(binwidth =  0.2,color='black', fill='#FFCC67') + xlim(3.8, 14.2)
# limit the upper value of x-axis to its 95 perecentile
ggplot(aes(x = fixed.acidity), data = wht_wine_quality) + geom_histogram(binwidth =  0.2,color='black', fill='#FFCC67') + xlim(3.8, quantile(wht_wine_quality$fixed.acidity,0.99))
...
# Histogram using 'qplot'function
qplot(x =volatile.acidity, data = wht_wine_quality) + scale_x_continuous()
# Limit the x-axis to fit the data
ggplot(aes(x = citric.acid), data = wht_wine_quality) + geom_histogram(binwidth =  0.02,color='black', fill='#FFCC67') + xlim(0, 1.66)
...
# Histogram using 'qplot'function
qplot(x = citric.acid, data = wht_wine_quality) + scale_x_continuous()
# Limit the x-axis to fit the data
ggplot(aes(x = citric.acid), data = wht_wine_quality) + geom_histogram(binwidth =  0.02,color='black', fill='#FFCC67') + xlim(0, 1.66)
# limt the upper value of xlim to a 99 percentile

ggplot(aes(x = citric.acid), data = wht_wine_quality) + geom_histogram(binwidth =  0.02,color='black', fill='#FFCC67') + xlim(0, quantile(wht_wine_quality$citric.acid,0.99))
...
# Histogram using 'qplot'function
qplot(x = residual.sugar, data = wht_wine_quality) + scale_x_continuous()
# Limit the x-axis to reflect the range of values 
ggplot(aes(x = residual.sugar), data = wht_wine_quality) + geom_histogram(binwidth =  .25,color='black', fill='#FFBBAA') + xlim(0, quantile(wht_wine_quality$residual.sugar,0.95))
...
# Limit the x-axis to fit the data
ggplot(aes(x = chlorides), data = wht_wine_quality) + geom_histogram(binwidth =  .001,color='black', fill='#09BBAA') + xlim(0, quantile(wht_wine_quality$chlorides,0.95))
...
# Limit the x-axis to fit the data , the 95 percentile is used to limit the upper value
ggplot(aes(x = free.sulfur.dioxide), data = wht_wine_quality) + geom_histogram(binwidth =  2,color='black', fill='#09BBAA') + xlim(0,quantile(wht_wine_quality$free.sulfur.dioxide,0.95))
...
ggplot(aes(x = total.sulfur.dioxide), data = wht_wine_quality) + geom_histogram(binwidth =  2,color='black', fill='#079CDF') + xlim(0,300) 
...
# Limit the x-axis to fit the data
ggplot(aes(x = pH), data = wht_wine_quality) + geom_histogram(binwidth =  0.01,color='black', fill='#FFCC67') + xlim(2.7, 3.85)
...
# Histogram using 'qplot'function
qplot(x = sulphates, data = wht_wine_quality) + scale_x_continuous()
# Use 'ggplot' and limit the x-axis to fit the data , used 95 percentile to see clear #picture.

ggplot(aes(x = sulphates), data = wht_wine_quality) + geom_histogram(binwidth =  0.01,color='black', fill='#FFCC67') + xlim(0.22, quantile(wht_wine_quality$sulphates,.95))
...
# Histogram using 'qplot'function
qplot(x = quality, data = wht_wine_quality) +  scale_x_continuous(limits = c(1,10), breaks = seq(1,10,1)) 
...
```{r echo=FALSE}
cor.test(wht_wine_quality$quality, wht_wine_quality$fixed.acidity, method = 'pearson')
...
```{r echo=FALSE}
cor.test(wht_wine_quality$quality, wht_wine_quality$citric.acid, method = 'pearson')
...
```{r echo=FALSE}
cor.test(wht_wine_quality$quality, wht_wine_quality$residual.sugar, method = 'pearson')
...
```{r echo=FALSE}
cor.test(wht_wine_quality$quality, wht_wine_quality$chlorides, method = 'pearson')
...
```{r echo=FALSE}
cor.test(wht_wine_quality$quality, wht_wine_quality$free.sulfur.dioxide, method = 'pearson')
...
```{r echo=FALSE}
cor.test(wht_wine_quality$quality, wht_wine_quality$total.sulfur.dioxide, method = 'pearson')
...
```{r echo=FALSE}
cor.test(wht_wine_quality$quality, wht_wine_quality$sulphates, method = 'pearson')
...
```{r echo=FALSE}
ggplot(aes(x=quality, y = alcohol), data = wht_wine_quality) + geom_point(alpha = 0.1, position = position_jitter(h=0), color='orange') + 
...
lnr_mod <-lm(formula=quality~alcohol+chlorides+citric.acid+density+fixed.acidity+free.sulfur.dioxide+pH+residual.sugar+sulphates+total.sulfur.dioxide+volatile.acidity)
...
qplot(quality, data = wht_wine_quality, binwidth = 0.5, main = "White Wine Quality") +
scale_x_continuous(limits = c(2,8.5), breaks = seq(2,8.5,1))  + xlab('Quality') +  ylab('Wine Count') 
...
ggplot(data = wht_wine_quality, aes(x = alcohol, y = density, color = factor(quality))) +
   geom_point(alpha = 1/2, position = position_jitter(h = 0), size = 1) +
   coord_cartesian(ylim = c(0.987, 1.003)) +
   scale_color_brewer(type = 'qual') + geom_smooth(method = "lm", se = FALSE, size = 1) +
   xlab('alcohol') +
   ylab('density') +
...
### The challenge while I was working on this data anysis was to identify and figure out which relation, pattern , or correlation among the varialbes is the most important. Which kind of plot I should you to visualize the relation and find out patterns was the other challenge. I understand data before doing any kind of analysis on a given dataset , one should have a good deal of understanding about the dataset. This would have made it much easier to figure out or sepeculate relationships, patterns or trends among each varaible of interest. In my future work of data analysis, I would first try to understand the dataset and vararaibles in the dataset before I dive into analysis.  
